...
export const validateUserData = async (
  ...
  if (userEmail && userEmail.status == 1)
    return next(new Error('USER_EMAIL_ALREADY_EXIST'));

  // 通过邮箱判断账号状态如果为 0 ,并且 距离创建时间 ＞ 1800秒, 则删除此未完成注册记录
  if (
    userEmail &&
    userEmail.status == 0 &&
    create_time - userEmail.create_time > 1800
  ) {
  ...
  if (
    ...
    return next(new Error('USER_EMAIL_WAITING_VERIFICATION'));
  ...
  if (
    userName &&
    userName.status == 0 &&
    create_time - userName.create_time > 1800
  ) {
...
export const validateUpdateUserData = async (
  ...
  try {
    ...
    if (update.name || update.password) {
      ...
      if (!matched) {
        return next(new Error('OLD_PASSWORD_DOES_NOT_MATCH'));
      }
    }
