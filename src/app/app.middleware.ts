...
export const defaultErrorHandler = (
  ...
  switch (error.message) {
    ...
    case 'PASSWORD_IS_THE_SAME':
      statusCode = 400
      message =
        'The password to be changed cannot be the same as the old password' // 要修改的密码不能跟原密码一样
      break
    ...
    case 'USER_EMAIL_WAITING_VERIFICATION':
      statusCode = 409
      message =
        'Your account is waiting to be activated! The activation link has been sent to your email, please activate within 30 minutes after registration' // 您的账户正在等待激活!激活链接已发送到您的邮箱,请在注册起30分钟内进行激活
      break
    case 'PLEASE_RE-REGISTER_BECAUSE_OF_TIMEOUT_ACTIVATION':
      statusCode = 409
      message =
        'Please register again because activation of this account has timed out' // 由于此账号激活超时,请重新注册
      break
    ...
    case 'FORBIDDEN':
      statusCode = 403
      message = 'no access' // 没有权限访问
      break
    case 'TOKEN_TYPE_IS_ADMIN_CANNOT_BE_USED_FOR_USER_REQUESTS':
      statusCode = 403
      message =
        'Your Token type is administrator and cannot be used for requests from ordinary users' // 您的Token类型为管理员,不能用于普通用户的请求
      break
